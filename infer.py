...
def evalDataset(setName='validation'):
    if(setName not in sets): raise AssertionError("invalid setName " + setName)
    processor = getProcessor(TUNED_MODEL_PATH)
    corr = 0
    for vset in ds[setName]:
        infer = inferByPixel(processor(vset['image'], return_tensors='pt')['pixel_values'])
        if infer == vset['labels']: corr += 1
        else: print(f'[{setName}] misclassificated {labels[vset['labels']]} as {labels[infer]}')
